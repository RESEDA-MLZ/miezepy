...
class Test_fit_library(unittest.TestCase):
    ...
    def test_sinus_fit_0(self):
        ...
        self.assertLessEqual(
            abs(self.results.getLastResult(name='Fit Data Sinus', key='amplitude') - 50), 1)
        self.assertLessEqual(
            abs(self.results.getLastResult(name='Fit Data Sinus', key='mean') - 100), 1)
        self.assertLessEqual(
            abs(self.results.getLastResult(name='Fit Data Sinus', key='phase') - 0.5*np.pi), 0.1)
    ...
    def test_tau_calculation_0(self):
        self.assertAlmostEqual(miezeTauCalculation(
            6e-9, 3, 6, 1e10)[0], 0.0828, 3)

    def test_tau_calculation_1(self):
        self.assertAlmostEqual(miezeTauCalculation(
            8e-9, 3, 6, 1e10)[0], 0.196, 3)
    ...
    def test_tau_processing_0(self):
        ...
        self.assertAlmostEqual(miezeTauProcessing(
            self.meta, self.data)[0], 0.0828, 3)
