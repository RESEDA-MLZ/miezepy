...
#         self.wavelength['probs'][:stepnumber /
#                                  2] = np.linspace(0, 2/self.wavelength['FWHM'], stepnumber/2)
#         self.wavelength['probs'][stepnumber /
#                                  2:] = np.linspace(2/self.wavelength['FWHM'], 0, stepnumber/2)
...
#             + 4*np.pi*self.instrument['freqs'][echo, 1] *
#             self.instrument['L1']*co.m_n/co.h*lambda_i*1e-10
...
#             self.calc_initialTime(
#                 2*(self.instrument['freqs'][freq, 1]-self.instrument['freqs'][freq, 0]), 200)
...
#             counts = np.histogram(t_channel, 16, range=(
#                 0, 16), weights=(np.cos(phase)+1)*probability)
...
#             # inelastic time
#             time_inel = self.detector_time_inelastic(
#                 time, wavelength, energytransfer)
...
#         lambda_i and initial start time into the instrument of t_A0
#         """
#         lambda_f = self.calc_wavelength_after_scattering(
#             lambda_i, energytransfer)
...
#             self.energytransfer['steps'],
#             self.initial_time['steps']))
#         energytransfer = energytransfer.swapaxes(
#             1, 2)*self.energytransfer['values']
...
#         probability = (probability.swapaxes(0, 2) *
#                        self.wavelength['probs']).swapaxes(0, 2)
#         probability = (probability.swapaxes(1, 2) *
#                        self.energytransfer['probs']).swapaxes(1, 2)
