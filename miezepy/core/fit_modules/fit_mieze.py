...
class Fit_MIEZE(FitHandler, PhaseProcessing, ContrastProcessing):
    ...
    def test_parameter(self, value, target, mask, results):
        ...
        if value == 'Select':
            ...
            if select[0] == 'all':
                ...
                try:
                    [target.get_axis(self.para_dict['para_name']).index(
                        selected) for selected in select]
                except:
        ...
        # perform the check on test
            ...
            if not len(foils_in_echo) == target.get_axis_len(self.para_dict['echo_name']):
                raise Exception(
                    'Not enough foils_in_echo initialized. Error...')
        ...
        # perform a test on BG
            ...
            if not BG == None and not BG in target.get_axis(self.para_dict['para_name']):
                raise Exception(
                    'The background is not in the loaded data. Error...')
        ...
        # perform a test on the reference
            ...
            if self.para_dict[value] in target.axes.names:
                ...
                raise Exception(
                    'The axis name does not exist in the provided data. Error...')
