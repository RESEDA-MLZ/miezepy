...
class DataStructure:
    ...
    def __str__(self):
        ...
        try:
            ...
            output += "- The base structure is: " + \
                str(self.data_objects[0].type)+"\n"
            output += "- The base structure dim is: " + \
                str(self.data_objects[0].dim)+"\n"
        except:
            output += "- No Axes defined\n"

        output += "- The metadata addition is set to: " + \
            str(self.add_meta_auto)+"\n"
        output += "----------------------------------------------------------\n"
    ...
    def get_metadata(self, index):
        ...
        metadata_array = [
            self.metadata_objects[self.metadata_addresses.index(
    ...
    def addDataObject(self, data, index):
        ...
        if self.add_meta_auto and not len(self.metadata_objects) < 1:
            self.data_objects[-1].meta_address.append(
                self.metadata_objects[-1].meta_id)
    ...
    def returnAsNumpy(self):
        ...
        extra_index = [slice(0, None, 1)
                       for i in list(self.data_objects[0].dim)]
        for data_object in self.data_objects:
    ...
    def process_reduction(self, index, id_array):
        ...
        for idx in id_array:
            new_data.inject_data_object(
                self.data_objects[self.get_pos_from_id(idx)])
        self.log.addLog(
            'info',
        ...
        for i in range(len(index)):
            if not mask[i]:
                new_data.axes.names[new_idx] = copy.deepcopy(
                    self.axes.names[i])
                new_data.axes.units[new_idx] = copy.deepcopy(
                    self.axes.units[i])
        ...
        for data_object in new_data.data_objects:
            for i in range(len(data_object.meta_address)):
                data_object.meta_address[i] = new_data.metadata_addresses[
    ...
    def clean_data(self, equivalence):
        ...
        for data_object in self.data_objects:
            ...
            for i in range(len(data_object.index)):
                ...
                data_object.index[i] = equivalence[i].index(
                    data_object.index[i])
...
class Metadata:
    ...
    def __iadd__(self, metadata_object):
        ...
        for key in self.metadata.keys():
            if self.metadata[key][1] == 'float':
                self.metadata[key][2] = str(
                    float(metadata_object.metadata[key][2]) + float(self.metadata[key][2]))
            elif self.metadata[key][1] == 'int':
                self.metadata[key][2] = str(
                    int(float(metadata_object.metadata[key][2])) + int(float(self.metadata[key][2])))
    ...
    def __truediv__(self, value):
        ...
        for key in self.metadata.keys():
            if self.metadata[key][1] == 'float':
                self.metadata[key][2] = str(
                    float(self.metadata[key][2]) / value)
            elif self.metadata[key][1] == 'int':
                self.metadata[key][2] = str(
                    int(float(self.metadata[key][2])) / value)
        return self
