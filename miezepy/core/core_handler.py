    ...
    def delEnv(self, key=None, idx=None):
        ...
        if not idx is None:
            ...
            if key in names:
                ...
                print("\nERROR: The key '"+str(key) +
                      "' you have provided is not present as an environment...\n")
    ...
    def saveSession(self, path, data_bool=True, mask_bool=False, script_bool=False):
        ...
        for key in names:
            ...
            if not os.path.exists(env_dir):
                os.makedirs(env_dir)
            self.env_array[names.index(key)].saveToPy(
                os.path.join(env_dir, "env_def.py"))
