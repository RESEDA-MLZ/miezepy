...
class Import_MIEZE_TOF:
    ...
    def process_tof_axes(self, container):
        ...
        for i in range(len(para_array)):
            for j in range(len(para_array[0][0])):
                real_idx_definition[j][i] = real_axis_definition[j].index(
                    temp_axis_definition[j][i])
    ...
    def fetch_data_tof(self, container, axes):
        ...
        for idx, name in enumerate(dimensions_names):
            try:
                ...
                try:
                    target.axes.set_axis(
                        idx, [float(element) for element in real_axis_definition[idx]])
                except:
                    target.axes.set_axis(
                        idx, [element for element in real_axis_definition[idx]])
            except:
                pass

        # set some metadata
        target.metadata_class.addMetadata(
        ...
        target.metadata_class.addMetadata(
            'Selected foils', value='[1,1,1,0,0,1,1,1]', logical_type='int_array', unit='-')
        target.metadata_class.addMetadata(
            'Resolution', value='[28.6, 0]', logical_type='float_array', unit='K')
                ...
                if len(line.split(element[1]+' :')) > 1:
                    try:
                        metadata[element[2]] = [
                            element[2],
                            element[3],
                            str(float(line.split(
                                element[1]+' : ')[1].split(element[-1])[0])*float(element[4])),
                            element[4]
                        ]
                    except:
                        metadata[element[2]] = [
